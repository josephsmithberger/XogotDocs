...
`-l`,`--language <locale>` | Use a specific locale.`<locale>`follows the format`language_Script_COUNTRY_VARIANT`where language is a 2 or 3-letter language code in
...
Command | Description
------- | -----------
`-s`,`--script <script>` | Run a script.`<script>`must be a resource path relative to the project (`myscript.gd`will be interpreted as`res://myscript.gd`)
...
`--gdscript-docs <path>` | Rather than dumping the engine API, generate API reference from the inline documentation in the GDScript files found in <path>
(used with`--doctool`).
