...
A [Viewport](https://docs.godotengine.org/en/stable/classes/class_viewport.html#class-viewport)'s child nodes will render to the image
...
or even directly to the [SceneTree](https://docs.godotengine.org/en/stable/classes/class_scenetree.html#class-scenetree).
...
1. Pro: RAM is no longer dragging the dead weight.

1. Con: Returning to that scene is now more expensive since it must be
loaded back into memory again (takes time AND memory). Not a problem
if returning soon is unnecessary.

1. Con: No longer have access to that scene's data. Not a problem if
using that data soon is unnecessary.

1. Note: It can be useful to preserve the data in a soon-to-be-deleted
scene by re-attaching one or more of its nodes to a different scene,
or even directly to the [SceneTree](https://docs.godotengine.org/en/stable/classes/class_scenetree.html#class-scenetree).
...
1. Pro: No nodes means no processing, physics processing, or input
handling. The CPU is available to work on the new scene's contents.

1. Con: Those nodes' processing and input handling no longer operate.
Not a problem if using the updated data is unnecessary.

1. **Hide the existing scene.** By changing the visibility or collision
...
1. Pro: You can still access the data if needed.

1. Pro: There's no need to move any more nodes around to save data.

1. Con: More data is being kept in memory, which will be become a problem
on memory-sensitive platforms like web or mobile.
...
1. Pro: Data continues to receive processing updates, so the scene will
keep any data within it that relies on delta time or frame data
updated.

1. Pro: Nodes are still members of groups (since groups belong to the
[SceneTree](https://docs.godotengine.org/en/stable/classes/class_scenetree.html#class-scenetree)).

1. Con: The CPU's attention is now divided between both scenes. Too much
load could result in low frame rates. You should be sure to test
performance as you go to ensure the target platform can support the
load from this approach.
...
or other data that is derived from [SceneTree](https://docs.godotengine.org/en/stable/classes/class_scenetree.html#class-scenetree)
access.




1. Memory still exists (similar pros/cons as hiding it from view).

1. Processing stops (similar pros/cons as deleting it completely).

1. Pro: This variation of "hiding" it is much easier to show/hide. Rather
than potentially keeping track of multiple changes to the scene, you
only need to call the add/remove_child methods. This is similar to
disabling game objects in other engines.

1. Con: Unlike with hiding it from view only, the data contained within
the scene will become stale if it relies on delta time, input, groups,
or other data that is derived from [SceneTree](https://docs.godotengine.org/en/stable/classes/class_scenetree.html#class-scenetree)
access.
