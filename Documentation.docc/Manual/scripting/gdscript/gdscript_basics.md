...
read this tutorial: <doc:gdscript_advanced>.
...
The following is the list of supported operators and their precedence. All binary operators are left-associative,
...
x is Nodex is not Node | Type checkingSee also [is_instance_of()](https://docs.godotengine.org/en/stable/classes/class_@gdscript_method_is_instance_of.html#class-@gdscript_method_is_instance_of)
function.
x ** y | PowerMultipliesxby itselfytimes, similar to calling
...
x * yx / yx % y | Multiplication / Division / RemainderThe%operator is additionally used for
...
x in yx not in y | Inclusion checkinginis also used with theforkeyword as part of the syntax.
not x!x | Boolean NOT and its :ref:`unrecommended <boolean_operators>` alias
x and yx && y | Boolean AND and its :ref:`unrecommended <boolean_operators>` alias
x or yx || y | Boolean OR and its :ref:`unrecommended <boolean_operators>` alias
...
GDScript also supports <doc:gdscript_format_string>.
...
[GDScript class reference](https://docs.godotengine.org/en/stable/classes/class_@gdscript.html#class-@gdscript).
...
For more information about exporting properties, read the <doc:gdscript_exports>
article.
...
<doc:gdscript_documentation_comments> for details.
...
> the project settings. See <doc:warning_system> for details.
>
...
1. Change default to a single underscore.
...
1. idle.gd can have a different number of arguments than the base class state.gd.

1. In the example above, e passed to the state.gd constructor is the same e passed
in to idle.gd.

1. If idle.gd's _init constructor takes 0 arguments, it still needs to pass some value
...
See <doc:running_code_in_the_editor> for more information.
