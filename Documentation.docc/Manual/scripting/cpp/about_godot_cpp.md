...
godot-cpp are the official C++ GDExtension bindings, maintained
as part of the Godot project.

godot-cpp is built with the <doc:index>, which allows access to Godot in almost the
same way as <doc:custom_modules_in_cpp>: A lot of engine code
can be used in your godot-cpp project almost exactly as it is.

In particular, godot-cpp has access to all functions that <doc:index> and <doc:index>
...
You can use both godot-cpp
and <doc:custom_modules_in_cpp> to run C or C++ code in a Godot project.
...
`extension_api.json` files in the <doc:scons>.