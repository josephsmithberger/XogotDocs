...
- 4.x will enjoy a longer <doc:release_policy>. Godot 3.x
will continue to be supported for some time after 4.0 is released, but it will
eventually stop receiving support.

See <doc:docs_changelog> for a list of pages documenting new features in
Godot 4.0, and <doc:list_of_features> for a list of all features in Godot.
...
Software OpenGL implementations
...
[Tweener](https://docs.godotengine.org/en/stable/classes/class_tweener.html#class-tweener) and the [Time](https://docs.godotengine.org/en/stable/classes/class_time.html#class-time) singleton in your project. These
...
1. Import the Godot 3.x project using the **Import** button, or use the **Scan**
button to find the project within a folder.

1. Double-click the imported project (or select the project then choose **Edit**).

1. You will see a dialog appearing with two options: **Convert project.godot
...
1. Wait until the project conversion process finishes. This can take up to a few
minutes for large projects with lots of scenes.

1. When the Project Manager interface becomes available again, double-click the
...
To use the project upgrade tool from the <doc:command_line_tutorial>,
...
- File and Directory classes were replaced by [FileAccess](https://docs.godotengine.org/en/stable/classes/class_fileaccess.html#class-fileaccess) and
[DirAccess](https://docs.godotengine.org/en/stable/classes/class_diraccess.html#class-diraccess), which have an entirely different API. Several methods
...
Godot 3.x, you will have to change its code to call [RenderingServer](https://docs.godotengine.org/en/stable/classes/class_renderingserver.html#class-renderingserver)
...
See <doc:shading_language> for more information.
...
- <doc:index#Signals> was changed.
The conversion tool will use the string-based syntax which is still present in
Godot 4, but it's recommended to switch to the [Signal](https://docs.godotengine.org/en/stable/classes/class_signal.html#class-signal)-based syntax
...
BakedLightmap | LightmapGI | See <doc:using_lightmap_gi>.
BakedLightmapData | LightmapGIData
BitmapFont | FontFile | See <doc:gui_using_fonts>.
...
Navigation2D | Node2D | Replaced by <doc:navigation_introduction_2d>.
Navigation3D | Node3D | Replaced by <doc:navigation_introduction_3d>.
...
ProximityGroup | Node3D | [VisibleOnScreenNotifier3D](https://docs.godotengine.org/en/stable/classes/class_visibleonscreennotifier3d.html#class-visibleonscreennotifier3d) can act as a replacement.
...
<doc:using_multiple_threads> APIs have changed in 4.0. For
...
The editor/renames_map_3_to_4.cpp
source file lists all automatic renames performed by the project upgrade tool.
Lines that are commented out refer to API renames that <doc:upgrading_to_godot_4#Manual-Rename>.
...
be ignored by your <doc:version_control_systems>.