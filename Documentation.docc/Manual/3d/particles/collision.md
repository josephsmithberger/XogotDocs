...
@Image(source: "particle_collision.png", alt: "Particle collisions")
...
on each system's [visibility layers](https://docs.godotengine.org/en/stable/classes/class_visualinstance3d.html#class-visualinstance3d). A particle system collides with a
collision node only if at least one of the system's visibility layers is enabled in the
collider's cull mask.

## Box collision

@Image(source: "particle_collision_box_entry.png", alt: "Particle collision box") {Box collision in the node list}
...
@Image(source: "particle_collision_box.png", alt: "Box collision with particle systems") {Two particle systems collide with a box collision node}

## Sphere collision

@Image(source: "particle_collision_sphere_entry.png", alt: "Particle collision sphere") {Sphere collision in the node list}
...
@Image(source: "particle_collision_sphere.png", alt: "Sphere collision with particle systems") {Two particle systems collide with a sphere collision node}

## Height field collision

@Image(source: "particle_collision_height.png", alt: "Particle collision height field") {Height field collision in the node list}
...
@Image(source: "particle_heightfield_res.png", alt: "Height field resolutions") {At low resolutions, height field collision misses some finer details (left)}
...
@Image(source: "particle_collision_sdf_entry.png", alt: "Particle collision SDF") {SDF collision in the node list}

SDF collision nodes create a signed distance field
...
@Image(source: "particle_collision_sdf.png", alt: "SDF particle collision") {SDF particle collision allows for very detailed 3-dimensional collision shapes}
...
@Image(source: "particle_collision_sdf_res.png", alt: "Resolution comparison") {The same area covered by a signed distance field at different resolutions: 16 (left) and 256 (right)}
...
For particle collision to work, the particle's <doc:index#Properties-Draw>
