...
@Image(source: "particle_minmaxcurve.png", alt: "ParticleProcessMaterial properties") {Min, max, and curve properties}
...
> For information on how to use it in a 2D Scene see <doc:particle_process_material_2d>.
>
...
the same amount of time, set by the <doc:index#Properties-Time> property. A value of `1` means
...
property is the same as setting the <doc:index#Properties-Draw> property to
`Y to Velocity`.
...
is any particle that uses the <doc:standard_material_3d>
...
determined by the <doc:index#Properties-Draw> property.
...
@Image(source: "particle_shapes_simple.png", alt: "Simple particle emission shapes") {Particles emitting from a point (left), in a sphere (middle), and in a box (right)}

@Image(source: "particle_ring.png", alt: "Ring-shaped particle system") {A ring-shaped particle system}
...
<doc:index#Complex-Shapes> section for a detailed
...
the <doc:standard_material_3d>, if it is set to `Particle Billboard`, rotates
...
@Image(source: "particle_direction.png", alt: "Different values for particle direction") {Different direction values: Y-axis only (left), equal values for X and Y (middle), X and Y with gravity enabled (right)}
...
@Image(source: "particle_spread.png", alt: "Different values for particle spread") {No spread (left), 45 degree angle (middle), full 180 degrees (right)}
...
@Image(source: "particle_gravity.png", alt: "Different values for particle gravity") {Left: (X=0,Y=-9.8,Z=0), middle: (X=0,Y=9.8,Z=0), right: (X=4,Y=2,Z=0).}
...
or the `Particle Billboard` mode is selected in the <doc:standard_material_3d>.
...
@Image(source: "particle_accel_linear.png", alt: "Different values for particle linear acceleration") {Negative (top) and positive (bottom) linear acceleration}
...
@Image(source: "particle_accel_radial.png", alt: "Different values for particle radial acceleration") {Negative (left) and positive (right) radial acceleration}

### Tangential acceleration

@Image(source: "particle_tangent.png", alt: "Tangents on a circle") {Tangents on a circle}
...
@Image(source: "particle_accel_tangent.png", alt: "Tangential acceleration from above") {Tangential acceleration from above}
...
If you want the particle system to interact with <doc:index#Attractors>,
...
billboards, the `Keep Size` property on the <doc:standard_material_3d>
...
`Use As Albedo` property in the `Vertex Color` group of the <doc:standard_material_3d>
is enabled. This property is multiplied with color coming from the particle material's
own `Color` or `Texture` property.

@Image(source: "particle_ramp.png", alt: "Particle color ramp") {Setting up a color ramp}
...
@Image(source: "particle_hue.png", alt: "Different values for hue variation") {Different values for hue variation, both times with blue as base color: 0.6 (left) and 0.1 (right)}

### Animation

The `Animation` property group controls the behavior of sprite
sheet animations in the particle's <doc:standard_material_3d>.
...
@Image(source: "particle_sprite.png", alt: "A sprite sheet") {An 8x8 animated smoke sprite sheet}

1. Import a sprite sheet texture into the engine. If you don't have one at hand, you can download the :download:`high-res version of the example image <img/particle_sprite_smoke.webp>`.

1. Set up a particle system with at least one draw pass and assign a `Standard Material` to the mesh in that draw pass.

1. Assign the sprite sheet to the `Texture` property in the `Albedo` group

1. Set the material's `Billboard` property to `Particle Billboard`. Doing so makes the `Particles Anim` group available in the material.

1. Set `H Frames` to the number of columns and `V Frames` to the number of rows in the sprite sheet.

1. Check `Loop` if you want the animation to keep repeating.
...
@Image(source: "particle_animate.png", alt: "Animated particles") {Three different particle systems using the same smoke sprite sheet}
...
>
> .. math::
> AnimationFPS = frac{Numberofimages}{Lifetime}
>
...
@Image(source: "particle_animate_lifetime.png", alt: "Animated particles lifetimes") {The same particle system at different lifetimes: 1 second (left), 2 seconds (middle), 8 seconds (right)}
...
If the `Use Scale` property is enabled, the <doc:index#Properties-Collision>
...
You can learn more about particle collisions in the <doc:index#Collision>
section in this manual.

## Sub-emitter

@Image(source: "particle_sub_mode.png", alt: "Sub-emitter modes") {The available sub-emitter modes}
...
See the <doc:index#Subemitters> section in this manual for a detailed explanation of how
