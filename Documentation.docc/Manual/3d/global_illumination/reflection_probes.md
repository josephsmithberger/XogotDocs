...
more accurate reflections than <doc:using_voxel_gi> and
<doc:using_sdfgi> while being fairly cheap on system resources.

Since reflection probes can also store ambient light, they can be used as a
low-end alternative to VoxelGI and SDFGI when <doc:using_lightmap_gi> aren't viable (e.g. in procedurally generated levels).
...
@Image(source: "gi_none.png", alt: "Reflection probe disabled. Environment sky is used as a fallback.") {Reflection probe disabled. Environment sky is used as a fallback.}

@Image(source: "gi_none_reflection_probe.png", alt: "Reflection probe enabled.") {Reflection probe enabled.}

@Image(source: "gi_lightmap_gi_indirect_only_reflection_probe.png", alt: "Reflection probe enabled.") {Reflection probe enabled with LightmapGI used at the same time. The lightmap appears in the reflection.}
...
@Image(source: "reflection_probes_reflection_probe.png", alt: "Reflections in a room using ReflectionProbe only.") {Reflections in a room using ReflectionProbe only. Notice how small details
don't have any reflections.}

@Image(source: "reflection_probes_ssr.png", alt: "Reflections in a room using screen-space reflections only.") {Reflections in a room using screen-space reflections only. Notice how the
reflection on the sides of the room's walls is partly missing due to being
off-screen.}

@Image(source: "reflection_probes_reflection_probe_ssr.png", alt: "Reflections in a room using ReflectionProbe and screen-space reflections together.") {Reflections in a room using ReflectionProbe and screen-space reflections together.
...
reflections with shadows every frame. Fully <doc:using_lightmap_gi>
