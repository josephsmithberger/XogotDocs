...
As described in <doc:environment_and_post_processing>, Godot supports
...
cannot be customized with <doc:fog_shader>.
...
@Image(source: "volumetric_fog_comparison.png")
...
built-in FogMaterial or a custom ShaderMaterial (<doc:fog_shader>).
...
<doc:importing_images#Changing-Import-Type>.
...
@Image(source: "volumetric_fog_fog_material_density_texture.png", alt: "FogMaterial comparison (without and with density texture)") {Screenshot taken with **Volume Size** project setting set to 192 to make
...
FogVolume nodes' appearance can be customized using <doc:fog_shader>.
...
1. Create a new StandardMaterial3D in the mesh's **Material** property.

1. In the StandardMaterial3D, set **Shading > Shading Mode** to **Unshaded**,
**Billboard > Mode** to **Enabled**, enable **Proximity Fade** and set
**Distance Fade** to **Pixel Alpha**.

1. Set the **Albedo > Texture** to the texture below (right-click and choose **Save as…**):

@Image(source: "volumetric_fog_quad_mesh_texture.png")

1. After setting the albedo texture, go to the Import dock, select the texture
