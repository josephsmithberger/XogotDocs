...
> BiDI and Font Features demo project.
>
...
@Image(source: "using_fonts_bitmap_font_from_image_import_options.png", alt: "Changing import type to Font Data (Image Font)") {Changing import type to **Font Data (Image Font)**}
...
ASCII characters and follows standard ASCII ordering:

@Image(source: "using_fonts_monospace_bitmap_font_example.png", alt: "Bitmap font example") {Credit: LibreQuake
(scaled and cropped to exclude extended range)}

The following import options can be used to import the above font image
successfully:

@Image(source: "using_fonts_bitmap_font_from_image_example_configuration.png", alt: "Import options to use for the above example font") {Import options to use for the above example font}
...
@Image(source: "using_fonts_bitmap_font_advance_offsets_diagram.png", alt: "Diagram showing the advance and offset values being used in character ranges.") {Do note that the offsets can cause your text to be cropped off the edge of your label boundaries.}
...
@Image(source: "using_fonts_load_font.png") {Loading a font file}
...
to [Label3D](https://docs.godotengine.org/en/stable/classes/class_label3d.html#class-label3d).
...
@Image(source: "using_fonts_outline_example.png", alt: "Font outline example") {Font outline example}
...
[Label](https://docs.godotengine.org/en/stable/classes/class_label.html#class-label) and [RichTextLabel](https://docs.godotengine.org/en/stable/classes/class_richtextlabel.html#class-richtextlabel). Additionally, font shadows
...
@Image(source: "using_fonts_shadow.png", alt: "Configuring font shadow in a Label node") {Configuring font shadow in a Label node}

The result should look like this:

@Image(source: "using_fonts_shadow_example.png", alt: "Font shadow example") {Font shadow example}

> Tip:
>
> You can create local overrides to font display in Label nodes by creating a
> [LabelSettings](https://docs.godotengine.org/en/stable/classes/class_labelsettings.html#class-labelsettings) resource that you reuse across Label nodes. This
> resource takes priority over <doc:gui_skinning>.
>
...
@Image(source: "using_fonts_antialiasing_comparison.png", alt: "Font antialiasing comparison") {From top to bottom: Disabled, Grayscale, LCD Subpixel (RGB)}

> Note:
>
> Antialiasing cannot be changed on <doc:using_fonts_msdf>
...
@Image(source: "using_fonts_hinting_comparison.png", alt: "Font hinting comparison") {From top to bottom: None, Light, Full hinting}
...
> Import dock. This will use FreeType's autohinter
> to automatically add hinting instructions to the imported font.
>

### Subpixel positioning

Subpixel positioning can be adjusted. This is a FreeType
...
<doc:using_fonts_font_prerendering> can be used to alleviate this.
...
Google Fonts, try downloading the font from the
font author's official website instead.

@Image(source: "using_fonts_rasterized_vs_msdf_comparison.png", alt: "Comparison of font rasterization methods") {Comparison of font rasterization methods.
...
@Image(source: "using_fonts_msdf_import_options.png", alt: "Enabling MSDF in the font's import options") {Enabling MSDF in the font's import options}
...
<doc:using_fonts_font_fallbacks>) needs to include them.
Otherwise, emoji won't be displayed and placeholder "tofu" characters will
appear instead:

@Image(source: "using_fonts_emoji_placeholder_characters.png", alt: "Default appearance when trying to use emoji in a label") {Default appearance when trying to use emoji in a label}

After adding a font to display emoji such as
Noto Color Emoji,
you get the expected result:

@Image(source: "using_fonts_emoji_correct_characters.png", alt: "Correct appearance after adding an emoji font to the label") {Correct appearance after adding an emoji font to the label}

To use a regular font alongside emoji, it's recommended to specify a
<doc:using_fonts_font_fallbacks> that points to the
...
> Tip:
>
> Emoji fonts are quite large in size, so you may want to <doc:using_fonts_system_fonts> to provide emoji glyphs rather than
...
Tools like Fontello can be used to generate font
...
specify the icon font as a <doc:using_fonts_font_fallbacks>.
...
> Note:
>
> Several modern icon fonts such as Font Awesome 6
...
@Image(source: "using_fonts_advanced_import_settings.png") {Import dock}
...
@Image(source: "using_fonts_font_fallbacks_add.png") {Adding font fallback}

Click the dropdown arrow on the new element, then choose a font file using the
**Quick Load** or **Load** options:

@Image(source: "using_fonts_font_fallbacks_load.png") {Loading font fallback}
...
> <doc:using_fonts_opentype_font_features>, but this is not covered here
> for brevity reasons.
>

### Variable fonts

Godot has full support for variable fonts, which
...
To use a variable font, create a [FontVariation](https://docs.godotengine.org/en/stable/classes/class_fontvariation.html#class-fontvariation) resource in the
location where you intend to use the font, then load a font file within the
FontVariation resource:

@Image(source: "using_fonts_font_variation_create.png") {Creating a FontVariation resource}

@Image(source: "using_fonts_font_variation_load.png") {Loading a font file into the FontVariation resource}
...
@Image(source: "using_fonts_font_variation_variable_font.png") {List of variation axes}
...
increases distribution size. A single <doc:using_fonts_variable_fonts>
...
@Image(source: "using_fonts_faux_bold_italic_vs_real_bold_italic.png", alt: "Faux bold/italic (top), real bold/italic (bottom). Normal font used: Open Sans SemiBold") {Faux bold/italic (top), real bold/italic (bottom). Normal font used: Open Sans SemiBold}

Faux bold and italic is automatically used in [RichTextLabel](https://docs.godotengine.org/en/stable/classes/class_richtextlabel.html#class-richtextlabel)'s bold
...
but also to calculate the minimum size of controls such as [Label](https://docs.godotengine.org/en/stable/classes/class_label.html#class-label)
and [Button](https://docs.godotengine.org/en/stable/classes/class_button.html#class-button).
...
@Image(source: "using_fonts_advanced_import_settings_opentype_features.png") {OpenType feature overrides in Advanced Import Settings}

**In a specific font usage (FontVariation)**

To use a font feature, create a FontVariation resource like you would do for a
<doc:using_fonts_variable_fonts>, then load a font file
within the FontVariation resource:

@Image(source: "using_fonts_font_variation_create.png") {Creating a FontVariation resource}

@Image(source: "using_fonts_font_variation_load.png") {Loading a font file into a FontVariation resource}
...
@Image(source: "using_fonts_font_variation_opentype_features.png") {Specifying OpenType features in a FontVariation resource}

For example, here's the Inter font without the
Slashed Zero feature (top), then with the Slashed Zero OpenType feature enabled
(bottom):

@Image(source: "using_fonts_font_variation_slashed_zero.png") {OpenType feature comparison (Inter)}
...
@Image(source: "using_fonts_font_variation_disable_ligatures.png") {Disabling ligatures and kerning for a font}
...
<doc:using_fonts_emoji> section.

Create a [SystemFont](https://docs.godotengine.org/en/stable/classes/class_systemfont.html#class-systemfont) resource in the location where you desire to
use the system font:

@Image(source: "using_fonts_system_font_create.png") {Creating a SystemFont resource}
...
See <doc:runtime_file_loading_and_saving_fonts>
for details.
...
@Image(source: "using_fonts_advanced_import_settings_prerender_new_configuration.png", alt: "Adding a new prerendering configuration in the Advanced Import Settings dialog") {Adding a new prerendering configuration in the Advanced Import Settings dialog}
...
<doc:internationalizing_games>. Otherwise, stick to
...
@Image(source: "using_fonts_advanced_import_settings_prerender_translation.png", alt: "Enabling prerendering in the Advanced Import Settings dialog with the Glyphs from the Translations tab") {Enabling prerendering in the Advanced Import Settings dialog with the **Glyphs from the Translations** tab}
...
@Image(source: "using_fonts_advanced_import_settings_prerender_text.png", alt: "Enabling prerendering in the Advanced Import Settings dialog, Glyphs from the Text tab") {Enabling prerendering in the Advanced Import Settings dialog with the **Glyphs from the Text** tab}

> Tip:
>
> If your project supports <doc:internationalizing_games>,
...
@Image(source: "using_fonts_advanced_import_settings_prerender_character_map.png", alt: "Enabling prerendering in the Advanced Import Settings dialog, Glyphs from the Character Map tab") {Enabling prerendering in the Advanced Import Settings dialog with the **Glyphs from the Character Map** tab}
...
<doc:using_fonts_antialiasing> method used
for the default project font.

- **Default Font Hinting:** Controls the
<doc:using_fonts_hinting> method used for
