...
<doc:index> documentation for details.
...
> tick rate as described in the <doc:jitter_stutter#Input-Lag> section.
>
...
for the first time. See <doc:pipeline_compilations> for more information.
...
[ColorRect](https://docs.godotengine.org/en/stable/classes/class_colorrect.html#class-colorrect) node) so that it's not visible to the player.
...
using the highest possible setting and reducing in-game sensitivity to reduce mouse latency.
